...
RUN	apt-get update -y && apt-get upgrade -y && \
	apt-get install -y build-essential curl libxml2-dev autoconf zlib1g-dev libcurl4-openssl-dev \
	    libjpeg9-dev libpng-dev libfreetype6-dev pkg-config libmemcached-dev
...
RUN cd /opt && curl -L -o memcached-2.2.0.tgz https://pecl.php.net/get/memcached-2.2.0.tgz && \
